...
function decodeState(str) {
  try {
    const jsonStr = Buffer.from(decodeURIComponent(str), "base64").toString(
      "utf8"
    );
...
function getOAuthClient(overrides = {}) {
  ...
  const clientSecret =
    overrides.clientSecret || process.env.GOOGLE_CLIENT_SECRET;
  const redirectUri =
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      "Thiếu clientId/clientSecret. Cấu hình .env hoặc nhập thông tin env từ UI."
    );
  }
...
app.get("/callback", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .send(
        "Không thể trao đổi code để lấy token. Vui lòng thử lại.",
...
app.get("/tokens.json", (req, res) => {
  if (!lastTokens)
    return res
      .status(404)
      .json({ error: "Chưa có token nào. Hãy đăng nhập Google trước." });
  res.json(lastTokens);
});
